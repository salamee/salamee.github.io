...
context.fillText("A program that takes the action of arrow keys being inputted from user and moves their box/player around. This box is",10,50); 
context.fillText("moved to eat the smaller boxes that are generated randomly.The player can only eat these small boxes, and eating them will make he",10,60); 
context.fillText("players box marginally bigger. There are bigger boxes than the player and the smaller boxes, that the player cannot eat it, or else ",10,70);
context.fillText("it'll be game over. Initially these bigger boxes are clear with black outline, just before the user plays, to make them identifiable. ",10,80);
context.fillText("The user can eat these bigger boxes once they get bigger than the big boxes, from eating enough smaller boxes. When user eats all the ",10,90);
context.fillText("boxes on screen, the user wins. The ponts are clculated by subtracting the time take to play, and subtacting that from 150. 150 being ",10,100);
context.fillText("the max points, which would be impossible, since it is not possible to finish the game under 0 s.",10,110);
