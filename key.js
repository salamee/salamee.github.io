...
context.fillText("A program that takes the action of arrow keys being inputted from user and moves their box/player around. This box is \r\nmoved to eat the smaller boxes that are generated randomly.The player can only eat these small boxes, and eating them will make he \nplayers box marginally bigger. There are bigger boxes than the player and the smaller boxes, that the player cannot eat it, or else \nit'll be game over. Initially these bigger boxes are clear with black outline, just before the user plays, to make them identifiable. \nThe user can eat these bigger boxes once they get bigger than the big boxes, from eating enough smaller boxes. When user eats all the \nboxes on screen, the user wins. The ponts are clculated by subtracting the time take to play, and subtacting that from 150. 150 being \nthe max points, which would be impossible, since it is not possible to finish the game under 0 s.",10,50);
